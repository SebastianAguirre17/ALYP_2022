...
struct CSV {
	char cadena[100];
}Cadena1[39], Cadena2[39];

struct BatallaCSV {
	char battle_number[10];
	char name[50];
	char attacker_king[50];
	char defender_king[50];
	char location[50];
	char region[50];
	char attacker_outcome[50];
}BatallasCSV1[39], BatallasCSV2[39];
...
void QuitarSaltoDeLinea(char batallasCSV1[], CSV Cadena[]);
int TopeCadena(char Cadena[]);
...
void MostrarInformeBatallas(char batallasCSV1[], char batallasCSV2[]) {
	/*
	QuitarSaltoDeLinea(batallasCSV1, Cadena1);
	QuitarSaltoDeLinea(batallasCSV2, Cadena2);
	for (int i = 0; i < 39; i++) {
		cout << Cadena1[i].cadena << endl;
	}
	cout << endl << endl;
	for (int i = 0; i < 39; i++) {
		cout << Cadena2[i].cadena << endl;
	}
	*/
}

void QuitarSaltoDeLinea(char batallasCSV[], CSV Cadena[]) {
	for (int indiceBatallas = 0, indiceCadena = 0, indiceEstructura = 0; indiceBatallas <= TopeCadena(batallasCSV); indiceBatallas++) {
		if (batallasCSV[indiceBatallas] != '\n') {
			Cadena[indiceEstructura].cadena[indiceCadena] = batallasCSV[indiceBatallas];
			indiceCadena++;
		}
		else {
			Cadena[indiceEstructura].cadena[indiceCadena + 1] = '\0';
			indiceEstructura++;
			indiceCadena = 0;
		}
	}
...
/*void QuitarComa(CSV Cadena[], BatallaCSV BatallasCSV1[39]) {

}

void ParsearBatallaCSV(char batallasCSV1[], char batallasCSV2[]){
	QuitarSaltoDeLinea(batallasCSV1, Cadena1);
	QuitarSaltoDeLinea(batallasCSV2, Cadena2);
	QuitarComa(Cadena1, BatallaCSV BatallasCSV1);
	QuitarComa(Cadena2, BatallaCSV BatallasCSV2);
}
*/
