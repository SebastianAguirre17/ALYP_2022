...
void CopiarHataSeparador(char *origen, char *destino, char separador) {
    while ((*origen != separador) and (*destino++ = *origen++));
}
...
void Parsear(char* batallasCSV1, char* batallasCSV2) {
	
	for (int i = 0, pos = 0; i < 3; ++i) {
		char registro[TopeCadena];
		
		CopiarHataSeparador(batallasCSV1 + pos, registro, FinDeLinea);
		cout << registro << endl;
		pos += ContarCaracteres(registro);
		cout << batallasCSV1[pos] << endl;
		

		//pos += BuscarCaracter(batallasCSV1, FinDeLinea);
		//char registro = CopiarHataSeparador()
	}
}

void MostrarInformeBatallas(char batallasCSV1[], char batallasCSV2[]){
	Parsear(batallasCSV1, batallasCSV2);
}

void Pruebas() {
	Campo battle_number,name,attacker_king,defender_king;
	Copiar("battle_number",battle_number.valor);
	Copiar("name",name.valor);
	Copiar("attacker_king",attacker_king.valor);
	Copiar("defender_king",defender_king.valor);

	Registro registro;
	registro.cantidadCampos = 4;
	registro.campos[0] = battle_number;
	registro.campos[1] = name;
	registro.campos[2] = attacker_king;
	registro.campos[3] = defender_king;
